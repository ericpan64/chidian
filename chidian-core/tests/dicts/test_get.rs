...
fn test_nested_get() {
    ...
    // Test nested array mapping with flatten (create new instance)
    let flatten_options2 = p::options_with_flatten(true);
    let patient_dict_nums_flat = get(source_container.clone(), "data[*].patient.dicts[*].num", Some(flatten_options2)).unwrap().data;
...
fn test_get_apply() {
    ...
    let list_id_transformed = get(source_container.clone(), "list_data[0].patient.id", Some(chained_apply)).unwrap().data;
