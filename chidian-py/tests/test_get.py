...
class TestGetArrays:
    ...
    @pytest.mark.parametrize("path,indices", [
        ("list_data[0].patient", [0]),
        ("list_data[1].patient", [1]),
        ("list_data[-1].patient", [-1]),
    ])
    def test_single_index(self, simple_data: dict[str, Any], path: str, indices: list[int]):
    ...
    @pytest.mark.parametrize("path,slice_params", [
        ("list_data[1:3]", (1, 3)),
        ("list_data[1:]", (1, None)),
        ("list_data[:2]", (None, 2)),
        ("list_data[:]", (None, None)),
    ...
    def test_wildcard_on_list(self, list_data: list[Any]):
        ...
        assert get(list_data, "[:].patient.id") == [p["patient"]["id"] for p in list_data]
...
class TestGetEdgeCases:
    ...
    def test_nested_arrays(self, deep_nested_list: list[Any]):
        ...
        expected = [item["num"] for item in deep_nested_list[0]["patient"]["list_of_dicts"]]
        assert result == expected
