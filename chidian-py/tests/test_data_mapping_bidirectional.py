...
class TestDataMappingBidirectionalBasic:
    ...
    def test_mapping_invalid_models(self):
        ...
        with pytest.raises(
            TypeError, match="source_model must be a Pydantic v2 BaseModel"
        ):
            DataMapping(
                dict, Observation, mapping={"id": "subject_ref"}, bidirectional=True
            )

        with pytest.raises(
            TypeError, match="target_model must be a Pydantic v2 BaseModel"
        ):
            DataMapping(
                Patient, dict, mapping={"id": "subject_ref"}, bidirectional=True
            )

    def test_mapping_invalid_mappings(self):
        """Test mapping rejects non-string mappings."""
        with pytest.raises(
            TypeError, match="Bidirectional mappings must be string-to-string paths"
        ):
            DataMapping(
                Patient, Observation, mapping={123: "subject_ref"}, bidirectional=True
            )
...
class TestDataMappingBidirectionalForward:
    ...
    def test_forward_with_spillover(self):
        ...
        patient = Patient(
            id="123", name="John", active=True, internal_notes="sensitive", age=45
        )
        obs, spillover = mapping.forward(patient)
...
class TestDataMappingBidirectionalReverse:
    ...
    def test_reverse_nested(self):
        ...
        mapping = DataMapping(
            NestedSource,
            NestedTarget,
            mapping={"patient.id": "subject_id", "patient.name": "subject_name"},
            bidirectional=True,
        )
